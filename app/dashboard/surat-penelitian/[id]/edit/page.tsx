...
export default function Page({ params }: { params: { id: number } }) {
  ...
  const breadcrumbItems = [
    ...
    { title: 'Edit', link: `/dashboard/surat-penelitian/${id}/edit` }
  ];

  async function fetchData() {
    setIsLoading(true)
    try {
      const res = await ky(`/api/surat-penelitian/${Number(id)}`, { retry: { limit: 2 } });
    ...
    <ScrollArea className="h-[calc(100dvh-64px)]">
      <div className="flex-1 space-y-4 p-8">
        ...
        {isLoading ? (
          ...
          <SuratPenelitianForm initialData={data} />
        )}
      </div>
    </ScrollArea>
  );
