...
interface SuratPenelitianFormProps {
  initialData?: any;
}

export default function SuratPenelitianForm({ initialData }: SuratPenelitianFormProps) {
  const form = useForm<z.infer<typeof suratPenelitianSchema>>({
    resolver: zodResolver(suratPenelitianSchema),
  ...
  async function onSubmit(values: any) {
    ...
    try {
      ...
      const url = initialData ? `/api/surat-penelitian/${initialData.id}` : `/api/surat-penelitian`;
  ...
  const handleCloseModal = () => {
    ...
    if (isSuccess) {
      setTimeout(() => {
        router.push('/dashboard/surat-penelitian');
      }, 300);
    }
  };
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8" method="POST">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Nama Dosen Pembimbing</FormLabel>
                    <FormControl>
                      <Input placeholder="Tuliskan nama dosen pembimbing" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>NIP Dosen Pembimbing</FormLabel>
                    <FormControl>
                      <Input placeholder="Tuliskan NIP dosen pembimbing" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
