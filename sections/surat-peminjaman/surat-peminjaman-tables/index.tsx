...
import { useSuratPeminjamanTableFilters } from './use-surat-peminjaman-table-filters';
import { DataTableResetFilter } from '@/components/ui/table/data-table-reset-filter';
import { Input } from '@/components/ui/input';

export default function SuratPeminjamanTable() {
  const {
    searchQuery,
    setPage,
    setSearchQuery,
    ...
    isAnyFilterActive,
    resetFilters,
  } = useSuratPeminjamanTableFilters();

  const [data, setData] = useState<SuratPeminjaman[]>([]);
  ...
  async function fetchData() {
    ...
    try {
      ...
      const response = await ky(`/api/surat-peminjaman?${queryString}`, { retry: { limit: 2 } });
      if (!response.ok) {
