...
export const CellAction: React.FC<CellActionProps> = ({ data, onRefresh }) => {
  ...
  const onConfirm = async () => {
    setIsLoading(true);
    try {
      const res = await ky.delete(`/api/surat-peminjaman/${data.id}`, { retry: { limit: 2 } });
  ...
  async function print() {
    setIsLoading(true);
    try {
      const res = await ky(`/api/surat-peminjaman/${data.id}/word`, { retry: { limit: 2 } });
      const blob = await res.blob();

      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `Surat Peminjaman ${data.nim} - ${data.tanggal_pinjam}.docx`;
  ...
  return (
    <>
      ...
      <div className="flex gap-2">
        <Link
          href={`/dashboard/surat-peminjaman/${data.id}/edit`}>
